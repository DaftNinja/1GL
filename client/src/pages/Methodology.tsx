...
import {
  ...
  BookOpen, ArrowLeft, Wind, ExternalLink, BarChart3, FileText, AlertCircle,
} from "lucide-react";
...
const LIVE_API_SOURCES = [
  ...
  },
  {
    ...
    url: "energy-charts.info + marktstammdatenregister.de",
  ...
  },
  {
    ...
    url: "digital.iservices.rte-france.com",
...
const MARKET_INTEL_SOURCES = [
  { category: "Energy Economics", sources: ["IEA — Energy and AI 2025", "IEA 4E TCP — DC Energy Methodology Review 2025", "BP Energy Outlook 2025", "Goldman Sachs Research — AI Power Demand 2025", "Ember — Grids for Data Centres 2025", "ICIS — European DC Power Demand 2035"] },
  { category: "European Market Research", sources: ["DC Byte — Global Data Centre Index 2025", "S&P Global / 451 Research — European DC Power 2025", "KPMG Ireland — DC Market 2026", "AlgorithmWatch — Germany Grid Saturation 2025", "Addleshaws Goddard — Future Data Centres Germany 2025"] },
  { category: "Grid & Regulatory", sources: ["ENTSO-E Transparency Platform (Open Power System Data)", "DE-LU Electricity Market 2019–2025 (Kaggle)", "CRU/2025/236 — Ireland LEU Connection Policy", "EirGrid Generation Capacity Statement 2023–2032", "Elia Annual Report 2024", "NordREG — Nordic Market Report 2025"] },
  { category: "Technology Benchmarks", sources: ["OEP MODEX — Danish Energy Agency 2020 CAPEX/OPEX", "Siala 2020 — German Offshore Wind Expansion Model", "MaStR 2026 — Germany 31,986 wind turbines, 554 large consumers", "Bundesnetzagentur — Solar Q4-2024 by Bundesland"] },
  { category: "Country-Specific Intelligence", sources: ["NESO — UK Strategic Spatial Energy Plan (SSEP)", "REE PNIEC 2024 — Spain 81% renewables target", "PSE SA — Poland grid capacity data", "Fingrid — Finland Nordic integration", "NVE — Norway hydro reservoir statistics", "RTE Bilan Prévisionnel — France 2024", "GSE Annual Report — Italy 2023"] },
];

const HOW_IT_WORKS_STEPS = [
  { icon: Globe, title: "Select a Country", description: "Choose one of 14 European countries from the dropdown. Each country has a tailored dataset of live grid intelligence specific to its TSO, market structure, and DC landscape." },
  { icon: Database, title: "Live Data Aggregation", description: "Country-specific APIs are queried in real time — grid operator data, TSO publications, CSO statistics — and rendered as interactive charts alongside the AI report." },
  { icon: Brain, title: "AI System Prompt Enrichment", description: "Before every OpenAI call, the system prompt is pre-loaded with 62+ cited sources: ENTSO-E pricing, MaStR grid data, Ember energy mix, OEP technology benchmarks, DC market intelligence, and regulatory frameworks specific to that country." },
  { icon: FileText, title: "Structured Analysis Generation", description: "GPT-5.1 generates a validated JSON response covering power pricing, grid constraints, regulatory environment, DC power demand, location suitability scores, investor insights, and a market summary." },
  { icon: BarChart3, title: "Dashboard Assembly", description: "The structured AI output is rendered into the interactive dashboard. Data sources are server-injected (not AI-generated) to guarantee citation accuracy. Live grid charts update independently of the AI analysis." },
  { icon: ShieldCheck, title: "Analyst Review", description: "Power Trends is a decision-support tool for 1GigLabs analysts. All outputs are intended to supplement — not replace — primary research and expert judgment." },
];
                    ...
                    <div className="flex items-start gap-3 flex-1">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          ...
                          <span className="text-sm text-blue-700 font-medium">{src.tso}</span>
                          <Badge
                            variant="outline"
                            className={`text-xs ${src.badge === "Live" ? "border-green-200 text-green-700 bg-green-50" : src.badge === "Official" ? "border-blue-200 text-blue-700 bg-blue-50" : "border-slate-200 text-slate-600"}`}
                          >
                            {src.badge}
                          </Badge>
                        </div>
                        <p className="text-xs text-slate-400 mb-2 font-mono">{src.url}</p>
                        <p className="text-sm text-slate-600 leading-relaxed">{src.description}</p>
                      </div>
                    </div>
                    <div className="flex-shrink-0 text-right">
                      <p className="text-xs text-slate-400 uppercase tracking-wide mb-0.5">Refresh</p>
                      <p className="text-xs font-semibold text-slate-600">{src.refresh}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
